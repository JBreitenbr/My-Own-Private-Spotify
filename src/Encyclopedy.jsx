...
<Tooltip id={`my-anchor-element-${index}`}>
  <div className="flex flex-col mb-4"><div className="flex flex-row mx-2 mt-4 justify-center items-center" style={{width:"80vw"}}><img src={item.album_pic} className="h-32 w-32 sm:h-40 sm:w-40 lg:h-56 lg:w-56 m-2"/><div style={{minWidth:"55vw"}}><div className="text-base sm:text-xl mx-2 mt-2">{item.artist}</div><div className="text-base sm:text-xl mx-2">{item.album_name}</div><div className="text-base sm:text-xl mx-2">{item.album_date}</div><div className="mx-2 text-xs text-wrap">{item.genres}</div></div></div><div className="mt-2 text-xs" style={{fontSize:"9px"}}><div className="text-wrap flex"><div>{songs1.map((item)=><p className="text-wrap" style={{maxWidth:"50vw"}}>{item+")"}</p>)}</div><div className="ml-2">{songs2.map((item)=><p className="text-wrap">{item+")"}</p>)}</div></div></div></div>
