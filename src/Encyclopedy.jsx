...
<Tooltip id={`my-anchor-element-${index}`}>
  <div className="flex flex-col mb-4"><div className="flex flex-row mx-2 mt-4 justify-center items-center" style={{width:"75vw"}}><img src={item.album_pic} className="h-32 w-32 sm:h-40 sm:w-40 lg:h-56 lg:w-56 m-2"/><div><div className="text-xl sm:text-2xl mx-2 mt-2">{item.artist}</div><div className="text-base sm:text-xl mx-2">{item.album_name}</div><div className="text-base sm:text-xl mx-2">{item.album_date}</div><div className="mx-2 text-xs text-wrap">{item.genres}</div></div></div><div className="mx-2 mt-2 text-xs" style={{fontSize:"9px"}}><ol className="text-wrap">{item.album_tracks.split(")/").map((item,index)=><li>{item+")"}</li>)}</ol></div></div>
